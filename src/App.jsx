...
const App = () => {
  ...
  const initialArticles = [
    ...
    },
    {
      ...
      mediaType: 'image', content: '매년 수많은 별들이 뜨고 지지만, 유독 강렬한 빛을 낸 순간들이 있습니다. 글로벌 시상식을 휩쓸었던 아티스트의 눈물부터 화려한 레드카펫 패션까지 다시 만나보세요.',
    ...
    },
    {
      ...
      mediaType: 'image', content: '기나긴 미니멀리즘의 유행 속에서도 화려한 색채에 대한 인간의 갈망은 사라지지 않았습니다. 이번 봄, 당신의 아이덴티티를 색채로 표현해보세요.',
      size: 'small', lang: 'KR', isFeatured: false, style: { fontSize: '18px', fontWeight: '400', fontFamily: 'serif' }
    }
  ];
  ...
  const featuredArt = articlesList.find(a => a.isFeatured) || articlesList[0] || initialArticles[0];
  const gridArticles = articlesList.filter(a => a.id !== (featuredArt?.id || 0));
  ...
  const handleCMSPublish = async () => {
    if (!cmsForm.title || !cmsForm.content) { addToast('⚠️ 제목과 본문은 필수입니다.'); return; }
    ...
    if (editingId) {
      ...
      setArticlesList([newArt, ...updatedList]); addToast('🚀 발행 성공!');
    }
      ...
      <div className="min-h-screen bg-[#F8F9FA] text-black font-sans flex flex-col w-full overflow-x-hidden">
        <header className="px-6 md:px-10 py-6 border-b bg-white flex justify-between items-center sticky top-0 z-50 shadow-sm">
          <div className="flex items-center gap-4 md:gap-10">
            ...
            <div className="hidden md:flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-black text-yellow-400 flex items-center justify-center text-sm font-black">{loggedInUser?.name?.[0]}</div>
              <div className="text-[10px] font-black uppercase tracking-widest text-gray-500">Studio | {loggedInUser?.name}</div>
            </div>
          </div>
        ...
        <main className="flex-1 bg-white max-w-5xl mx-auto w-full p-6 md:p-24 animate-in fade-in my-10 rounded-[40px] md:rounded-[60px] shadow-2xl border border-gray-100 overflow-hidden">
          <div className="space-y-12">
            <select className="w-full bg-gray-50 border-2 border-gray-100 px-6 py-4 rounded-2xl font-black text-xs focus:border-black transition-all" value={cmsForm.category} onChange={e => setCmsForm({...cmsForm, category: e.target.value})}>
              {['NEWS', 'FASHION', 'INTERVIEW', 'LIFE', 'ARCHIVE'].map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <textarea placeholder="Headline..." className="w-full text-4xl md:text-6xl font-black italic border-none focus:ring-0 placeholder:text-gray-100 resize-none h-auto p-0 uppercase" value={cmsForm.title} onChange={e => setCmsForm({...cmsForm, title: e.target.value})} rows={2} />
            <input type="text" placeholder="Subtitle / 요약..." className="w-full text-xl md:text-2xl font-medium border-none focus:ring-0 italic p-0" value={cmsForm.subtitle} onChange={e => setCmsForm({...cmsForm, subtitle: e.target.value})} />
            ...
            <div className="border-2 border-gray-100 rounded-[30px] overflow-hidden bg-white shadow-sm focus-within:border-black transition-all">
              <div className="bg-gray-50 border-b-2 border-gray-100 p-6 flex flex-wrap gap-8 items-center">
                <div className="flex items-center gap-4 border-r pr-8">
                  ...
                  <span className="text-[11px] font-black w-10 text-center">{parseInt(cmsForm.style.fontSize)}px</span>
                </div>
                <div className="flex items-center gap-3 border-r pr-8">
                  <Bold size={16} className="text-gray-400" />
                  <select className="bg-white border border-gray-200 px-3 py-1 rounded-lg text-[10px] font-bold focus:border-black cursor-pointer" value={cmsForm.style.fontWeight} onChange={e => setCmsForm({...cmsForm, style: {...cmsForm.style, fontWeight: e.target.value}})}>
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setCmsForm({...cmsForm, style: {...cmsForm.style, fontFamily: 'serif'}})} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold border transition-all ${cmsForm.style.fontFamily === 'serif' ? 'bg-black text-white border-black shadow-lg' : 'bg-white text-gray-400 border-gray-200'}`}>Serif</button>
                  <button type="button" onClick={() => setCmsForm({...cmsForm, style: {...cmsForm.style, fontFamily: 'sans-serif'}})} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold border transition-all ${cmsForm.style.fontFamily === 'sans-serif' ? 'bg-black text-white border-black shadow-lg' : 'bg-white text-gray-400 border-gray-200'}`}>Sans</button>
                </div>
              </div>
              <textarea style={{ ...cmsForm.style }} placeholder="본문을 작성하세요..." className="w-full leading-relaxed border-none focus:ring-0 min-h-[600px] p-8 md:p-12 bg-transparent resize-y" value={cmsForm.content} onChange={e => setCmsForm({...cmsForm, content: e.target.value})} />
            </div>
          </div>
        </main>
      </div>
    ...
    if (!isAuthenticated) {
      return (
        <div className="min-h-screen bg-black flex items-center justify-center p-8 overflow-hidden w-full">
          <div className="max-w-md w-full bg-white rounded-[60px] p-16 shadow-2xl border-t-[16px] border-yellow-400 animate-in fade-in zoom-in">
            ...
            <form onSubmit={handleLogin} className="space-y-6 mt-12">
              ...
              <button type="submit" className="w-full bg-black text-white py-6 rounded-2xl font-black uppercase hover:bg-yellow-400 hover:text-black transition-all active:scale-95">Sign In</button>
            </form>
        ...
        <main className="flex-1 p-8 md:p-20 max-w-7xl mx-auto overflow-y-auto w-full h-screen">
          {adminTab === 'ARTICLES' ? (
             <div className="animate-in fade-in slide-in-from-bottom-8">
               <h2 className="text-5xl md:text-6xl font-black uppercase italic mb-16 tracking-tighter text-black font-black">Content Repository</h2>
               <div className="bg-white rounded-[40px] md:rounded-[50px] shadow-sm border border-gray-100 overflow-x-auto">
                 <table className="w-full text-left min-w-[800px]">
                   <thead className="bg-gray-50 text-[10px] font-black uppercase tracking-widest text-gray-400 border-b">
                     <tr><th className="p-10 text-black">Headline</th><th className="p-10 text-center text-black">Status</th><th className="p-10 text-center text-black">Actions</th></tr>
                   </thead>
                   <tbody>
                     {articlesList.map(a => (
                       <tr key={a.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors text-black">
                         ...
                         <td className="p-10 flex justify-center gap-4">
                           ...
                           <button onClick={() => { if(window.confirm('삭제하시겠습니까?')) { setArticlesList(articlesList.filter(art => art.id !== a.id)); addToast('🗑️ 삭제 완료'); } }} className="p-4 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-md active:scale-90"><Trash2 size={18}/></button>
                         </td>
                       </tr>
                     ))}
                   </tbody>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-8">
              ...
              <div className="bg-white rounded-[40px] md:rounded-[50px] shadow-sm border border-gray-100 p-10">
                <p className="text-xl font-bold mb-10 text-black">Total: <span className="text-yellow-500">{subscribers.length}</span></p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {subscribers.map((sub, i) => (
                    <div key={i} className="flex items-center justify-between p-8 border border-gray-100 rounded-[35px] bg-gray-50 group transition-all">
                      <div className="flex items-center gap-5">
                        <div className="w-14 h-14 bg-black text-white rounded-2xl flex items-center justify-center font-black shadow-lg">{sub.email[0].toUpperCase()}</div>
                        <div><p className="font-black text-lg text-gray-800">{sub.email}</p><p className="text-[10px] text-gray-400 font-bold uppercase">{sub.date}</p></div>
                      </div>
            ...
            <div className="animate-in fade-in space-y-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-black font-black">
                ...
                <div className="bg-black p-14 rounded-[40px] md:rounded-[60px] shadow-2xl text-white flex flex-col justify-between border-t-[20px] border-yellow-400 transition-transform hover:-translate-y-2">
                  <p className="text-[11px] font-black uppercase text-yellow-400 tracking-widest mb-10">Connectivity</p>
  ...
  if (selectedArticle) {
    return (
      <div className="animate-in fade-in duration-700 bg-white text-[#111] min-h-screen font-sans selection:bg-yellow-400 selection:text-black w-full overflow-x-hidden relative flex flex-col">
        ...
        <main className="flex-1 w-full pt-32 md:pt-40 pb-40 max-w-[1440px] mx-auto px-6 lg:px-12 grid grid-cols-12 gap-10 md:gap-12 relative h-auto">
          {/* 📍 [수정] 본문 그리드 비중 확대 및 여백 축소 📍 */}
          <div className="col-span-12 lg:col-span-8 xl:col-span-9 pr-0 lg:pr-10 border-r-0 lg:border-r border-gray-100 h-auto overflow-visible">
            ...
            <div className="flex flex-wrap items-center justify-between border-y border-gray-100 py-6 mb-12 text-gray-500 text-[10px] md:text-xs font-bold gap-6">
              ...
              <div className="flex items-center gap-6">
                 <button onClick={() => handleShare('facebook', selectedArticle)} className="text-gray-400 hover:text-[#1877F2] transition-all hover:scale-110"><Facebook size={20}/></button>
                 <button onClick={() => handleShare('twitter', selectedArticle)} className="text-gray-400 hover:text-[#1DA1F2] transition-all hover:scale-110"><Twitter size={20}/></button>
                 <button onClick={() => handleShare('link', selectedArticle)} className="text-gray-400 hover:text-black transition-all hover:scale-110"><LinkIcon size={20}/></button>
              </div>
            </div>

            {/* 📍 [복구] AI 핵심 요약 리포트 📍 */}
            {selectedArticle.summary && (
              <div className="bg-[#fcfcfc] border-2 border-gray-100 rounded-[35px] p-8 md:p-12 mb-12 shadow-sm relative overflow-hidden group hover:border-yellow-400 transition-all">
                <div className="absolute top-0 left-0 w-2 h-full bg-yellow-400"></div>
                <div className="flex items-center gap-3 mb-6 font-black">
                  <div className="w-8 h-8 bg-black text-yellow-400 rounded-xl flex items-center justify-center text-xs">AI</div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-10 p-12 md:p-16 bg-[#F8F9FA] rounded-[60px] mb-20 border border-gray-100 relative group overflow-hidden text-center md:text-left h-auto">
          ...
          {/* 사이드바 크기 최적화 (xl:col-span-3) */}
          <div className="hidden lg:block col-span-4 xl:col-span-3 space-y-16 sticky top-40 h-fit">
             <div className="bg-[#111] rounded-[60px] p-10 shadow-2xl border border-white/5 overflow-hidden relative group">
                ...
                <div className="space-y-10">
                   {articlesList.filter(a => a.id !== selectedArticle?.id).slice(0,4).map((trend, i) => (
                     <div key={trend.id} onClick={() => openArticle(trend)} className="flex gap-6 group cursor-pointer items-start transition-all hover:translate-x-2">
                        ...
                        <div>
                          <p className="text-[14px] font-bold text-gray-200 leading-tight group-hover:text-white transition-colors line-clamp-2 uppercase">"{trend.title}"</p>
  ...
  return (
    <div className="bg-black text-white font-sans selection:bg-yellow-400 selection:text-black w-full overflow-x-hidden relative flex flex-col min-h-screen">
      ...
      {/* 히어로 섹션 여백 최적화 (pt-40 / min-h-85vh) */}
      ...
      <footer className="bg-white text-black pt-32 md:pt-40 pb-24 px-6 md:px-12 text-center border-t border-black w-full overflow-hidden relative">
        <div className="overflow-hidden w-full h-auto">
          <h2 className="text-[25vw] font-black italic tracking-tighter leading-[0.7] mb-20 md:mb-24 uppercase select-none pointer-events-none opacity-90 transition-opacity hover:opacity-100 whitespace-nowrap">CELEBRITY <span className="text-yellow-400">MAG.</span></h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 md:gap-20 items-center border-t border-black/5 pt-20 md:pt-24">
          ...
          <div className="flex justify-center gap-10 md:gap-20 font-black uppercase text-[10px] md:text-[11px] tracking-[0.6em]">
            ...
            <a href="#admin" onClick={(e) => { e.preventDefault(); window.location.hash = 'admin'; setView('ADMIN'); window.scrollTo(0,0); }} className="hover:text-yellow-500 transition-all hover:scale-110 border-b border-black font-black">Staff Portal</a>
      ...
      {isSubscribeOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-500">
          ...
          <div className="relative bg-[#0a0a0a] border border-white/10 text-white max-w-3xl w-full p-12 md:p-32 rounded-[50px] md:rounded-[100px] shadow-[0_0_150px_rgba(250,204,21,0.2)] text-center overflow-hidden">
            {isSubscribed ? (
              <div className="animate-in zoom-in duration-500"><CheckCircle2 size={100} className="mx-auto text-yellow-400 mb-10 shadow-2xl" /><h2 className="text-5xl md:text-6xl font-black italic tracking-tighter uppercase mb-6 leading-none font-black text-white">접근 승인 완료.</h2><p className="text-gray-400 font-bold uppercase tracking-widest text-sm">이제 핵심 멤버(Inner Circle)입니다.</p></div>
            ) : (
              <div>
